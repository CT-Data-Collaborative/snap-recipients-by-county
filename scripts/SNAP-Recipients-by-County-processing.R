...
##################################################################
#
# Processing Script for SNAP Recipients by County
...
for (i in seq_along(yearlist)) {
  ...
  for (j in seq_along(tables)) {
    ...
    year <- paste(year-4, year, sep="-")
    geo <- data@geography
    #geo$county <- sprintf("%02s", geo$county)
...
county_data <- merge(counties, county_data, by="FIPS", all.x=T)

county_data <- county_data[county_data$County != "Connecticut",]

state_data$County <- "Connecticut"
...
totals_calc <- totals_calc %>% 
  select(FIPS, Year, `Measure Type`, `Race/Ethnicity`, County, `SNAP Recipients_Received`, `Margins of Error_Received`, pct_est, pct_moe) %>% 
...
dataset_final <- dataset_final %>% 
  ...
  arrange(County, FIPS, Year, `Race/Ethnicity`, `Measure Type`)
...
write.table(
    dataset_final,
    file.path("data", "snap-recipients-by-county-2018.csv"),
    sep = ",",
    row.names = F,
    na = "-9999"
)
